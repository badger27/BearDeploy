...
angular.module('inflightApp.templates', []).run(['$templateCache', function ($templateCache) {
  $templateCache.put('bower_components/angular-moment-picker/src/template.tpl.html', '<div class=moment-picker><div class="moment-picker-container {{view.selected}}-view" ng-class="{\'moment-picker-disabled\': disabled, open: view.isOpen}"><div ng-if=additions.top class="moment-picker-addition top"></div><table class=header-view ng-if=showHeader><thead><tr><th ng-class="{disabled: !view.previous.selectable}" ng-bind-html=view.previous.label ng-click=view.previous.set()></th><th ng-bind=view.title ng-click=view.setParentView()></th><th ng-class="{disabled: !view.next.selectable}" ng-bind-html=view.next.label ng-click=view.next.set()></th></tr></thead></table><div class=moment-picker-specific-views><table><thead ng-if=views[view.selected].headers><tr><th ng-repeat="header in views[view.selected].headers" ng-bind=header></th></tr></thead><tbody><tr ng-repeat="row in views[view.selected].rows"><td ng-repeat="item in row track by item.index" ng-class=item.class ng-bind=item.label ng-click="!disabled && views[view.selected].set(item)"></td></tr></tbody></table></div><div ng-if=additions.bottom class="moment-picker-addition bottom"></div></div></div>');
  $templateCache.put('components/player/poolBear/_partials/filterDropdown.tpl.html', '{{filterSelection}} <button ng-model=filterSelection href=# data-dropdown=drop1 aria-controls=drop1 aria-expanded=false class="button dropdown">filter</button><ul id=drop1 data-dropdown-content class=f-dropdown aria-hidden=true><li><a ng-click="filterByDropdownCategory(0, \'A-Z\')">A-Z</a></li><li ng-repeat="filter in playerFilterDropdown"><a ng-click="filterByDropdownFilter(filter.filterId, filter.filterName)">{{filter.filterName}}</a></li></ul><script>\n  $(document).foundation();\n</script>');
}]);
var poolBear = angular.module('poolBear', ['firebase', 'ui.router', 'poolBear.login', 'poolBear.auth', 'poolBear.result', 'poolBear.settings', 'poolBear.player', 'poolBear.actionmodal', 'poolBear.scoreMessage', 'poolBear.Message', 'poolBear.uploadFile', 'poolBear.Images', 'poolBear.uploadFileService', 'poolBear.search', 'poolBear.challenge', 'poolBear.group', 'poolBear.geo', 'poolBear.api', 'poolbear.confirmServices', 'services.menu']).run(
...
.constant('FIREBASE_URL', 'https://thebear.firebaseio.com/').config(['$stateProvider', '$urlRouterProvider', '$locationProvider', function ($stateProvider, $urlRouterProvider, $locationProvider) {
...
}]).controller('BearCtrl', ['$scope', '$rootScope', 'FIREBASE_URL', '$filter', '$location', '$state', '$stateProvide', function ($scope, $rootScope, FIREBASE_URL, $filter, $location, $state, $stateProvide) {
...
'use strict';
angular.module('poolBear.geo', []).config(function ($stateProvider) {

  $stateProvider.state('app.geo', {
...
'use strict';
angular.module('poolBear.challenge', []).config(function ($stateProvider) {
...
'use strict';
angular.module('poolBear.chat', []).config(function ($stateProvider) {
...
'use strict';
angular.module('poolBear.group', []).config(function ($stateProvider) {
...
'use strict';
angular.module('poolBear.login', ['ngFileUpload', 'poolBear.auth']).config(function ($stateProvider, $urlRouterProvider) {
  ...
  $stateProvider.state('app.home', {
    ...
    views: {
      'main': {
        ...
        resolve: {
          currentAuth: function currentAuth(Authentication) {
...
}).controller('SuccessController', ['$scope', '$rootScope', '$firebaseAuth', '$firebaseObject', "$firebaseArray", 'FIREBASE_URL', 'fileUpload', 'Upload', '$timeout', '$window', function ($scope, $rootScope, $firebaseAuth, $firebaseObject, $firebaseArray, FIREBASE_URL, fileUpload, Upload, $timeout, $window) {
...
'use strict';
angular.module('poolBear.Images', ['ngFileUpload']).config(function ($stateProvider, $urlRouterProvider) {
...
}).controller('imagesController', ['$scope', '$rootScope', '$state', '$location', '$firebaseAuth', '$firebaseObject', "$firebaseArray", 'FIREBASE_URL', 'fileUpload', 'Upload', '$timeout', '$window', function ($scope, $rootScope, $state, $location, $firebaseAuth, $firebaseObject, $firebaseArray, FIREBASE_URL, fileUpload, Upload, $timeout, $window) {
...
'use strict';
angular.module('poolBear.player', ['poolbear.confirmServices', 'moment-picker']).config(function ($stateProvider) {
...
.controller('PlayerController', ['$scope', '$rootScope', '$firebaseAuth', '$firebaseArray', '$firebaseObject', 'FIREBASE_URL', 'firebaseDataService', function ($scope, $rootScope, $firebaseAuth, $firebaseArray, $firebaseObject, FIREBASE_URL, firebaseDataService) {
...
}]).controller('DetailsCtrl', ['$scope', '$rootScope', '$firebaseAuth', '$firebaseObject', 'FIREBASE_URL', '$stateParams', '$state', function ($scope, $rootScope, $firebaseAuth, $firebaseObject, FIREBASE_URL, $stateParams, $state) {
...
'use strict';
angular.module('poolBear.result', ['poolBear.login', 'poolBear.player']).config(function ($stateProvider) {
...
}).controller('ResultCtrl', ['$scope', '$rootScope', '$firebaseAuth', '$firebaseArray', 'FIREBASE_URL', '$firebaseObject', '$stateParams', '$state', 'confirmScoreService', function ($scope, $rootScope, $firebaseAuth, $firebaseArray, FIREBASE_URL, $firebaseObject, $stateParams, $state, confirmScoreService) {
  void 0;
...
'use strict';
angular.module('poolBear.search', []).config(function ($stateProvider) {
...
(function () {
  ...
  angular.module("poolBear.auth", []).factory('Authentication', ['$rootScope', '$firebaseAuth', '$firebaseObject', '$location', 'FIREBASE_URL', function ($rootScope, $firebaseAuth, $firebaseObject, $location, FIREBASE_URL) {
    ...
    return {
      ...
      login: function login(user) {
        ...
        }).then(function (regUser) {}).catch(function (error) {
...
(function () {
  ...
  angular.module('services.bears', []).factory('selector', ['$http', function () {

    //SET Layout 
    var setLayout = function setLayout() {};

    //SET Layout 
    var setBearclass = function setBearclass() {};
...
.factory('interchangeTools', ['$window', 'matchMedia', 'interchangeQueries', function ($window, matchMedia, namedQueries) {
   ...
   */
  var parseAttribute = function parseAttribute(value) {
    var raw = value.split(/\[(.*?)\]/),
        i = raw.length,
        breaker = /^(.+)\,\ \((.+)\)$/,
        breaked,
   ...
   */
  var findCurrentMediaFile = function findCurrentMediaFile(files) {
    ...
    compile: function compile($element, attrs) {
      ...
      return {
        ...
        post: function postLink($scope, $element, attrs) {
          ...
          var replace = function replace(e) {
...
angular.module("mm.foundation.topbar", ['mm.foundation.mediaQueries']).factory('closest', [function () {
  ...
  return {
    ...
    link: function link(scope, element, attrs) {
      ...
      var updateStickyPositioning = function updateStickyPositioning() {
  ...
  return {
    ...
    transclude: true,
    link: function link(scope, element, attrs, topBar) {
...
(function () {
  ...
  angular.module('services.onboard', ['ngCookies']).factory('onboard', ['$http', '$cookieStore', '$location', onboard]);
  ...
  function onboard($http, $cookieStore, $location) {
    ...
    var getOnboardList = function getOnboardList() {
      var onboardData = [{ id: 0, name: "_movies", link: "app.movies.list" }, { id: 1, name: "_tv", link: "app.tvs.list"
        //          {  id:2, name: "_audios",link:"app.audios.list" }
      }];
      return onboardData;
    };

    var setCurrentLabel = function setCurrentLabel(onboardID, onboardName) {
    ...
    var getCurrentLabel = function getCurrentLabel() {
...
(function () {
  ...
  function bearApi($rootScope, $http, $q, $firebaseAuth, $firebaseObject, FIREBASE_URL) {
    ...
    var getPlayersfireBase = function getPlayersfireBase() {

      var userRef = new Firebase('https://thebear.firebaseio.com/users');

      return $http.get(userRef).then(function (response) {
...
(function () {
  ...
  angular.module('services.menu', []).factory('menuService', function () {
    var showMenu = false;
...
(function () {
  ...
  angular.module('poolBear').factory('themeService', function () {
    ...
    var getTheme = function getTheme(callback) {
      void 0;
      callback(theme);
    };
...
'use strict';
angular.module('poolBear.settings', []).config(function ($stateProvider) {
...
}).controller('SettingsCtrl', ['$scope', '$rootScope', 'Authentication', '$state', '$location', '$firebaseArray', 'FIREBASE_URL', '$firebaseObject', '$firebaseAuth', function ($scope, $rootScope, Authentication, $state, $location, $firebaseArray, FIREBASE_URL, $firebaseObject, $firebaseAuth) {
...
angular.module("themeConfig", []).constant("pool", "_poolBear").constant("languageID", " 221").constant("languageCode", "en").constant("deployPath", "/Volumes/Macintosh HD 2/dev/microsoft-staging/BearDeploy/").constant("deployDist", "/Volumes/Macintosh HD 2/dev/The-Bear/dist/").constant("themeData", ["'poolBear'", "'footballBear'", "'SquashBear'", "'golfBear'", "'cSBear'", "'tennisBear'"]).constant("ratingSystem", { "standard": "_standard", "advanced": "_advanced", "custom": "_custom" }).constant("app", ["'firebase'", "'ui.router'", "'poolBear.login'", "'poolBear.auth'", "'poolBear.result'", "'poolBear.settings'", "'poolBear.actionmodal'", "'poolBear.scoreMessage'", "'poolBear.Message'", "'poolBear.uploadFile'", "'poolBear.Images'", "'poolBear.Images'", "'poolBear.uploadFileService'", "'poolBear.search'", "'poolBear.challenge'", "'poolBear.group'", "'poolBear.geo'", "'poolBear.chat'", "'poolBear.api'", "'poolbear.confirmServices'", "'services.menu'"]);

'use strict';

angular.module('poolBear.actionmodal', ['ngAnimate']).directive('modal', function () {
  return {
    restrict: 'E',
    scope: {
      show: '='
    ...
    template: "<div class='ng-modal' ng-show='show'>" + "<div class='reveal-modal' ng-show='show'>" + "<div ng-transclude></div>" + "<a class='close-reveal-modal' ng-click='hideModal()'>&#215;</a>" + "</div>" + "<div class='reveal-modal-bg' ng-click='hideModal()'></div>" + "</div>"
  };
...
angular.module("poolBear.Message", []).controller('messageCtrl', function ($scope) {
...
angular.module("poolBear.PopMenu", []).controller('PopMenuCtrl', function ($scope) {
...
angular.module('poolBear.scoreMessage', ['ngAnimate']).directive('modalscoreMessage', function () {
  return {
    restrict: 'E',
    scope: {
      show: '='
...
angular.module('poolBear.uploadFile', []).directive('fileModel', ['$parse', function ($parse) {
  return {
    restrict: 'A',
    link: function link(scope, element, attrs) {
...
(function () {
  angular.module('inflightApp.jcarousel', []).directive('carousel', carousel).controller('tvController', tvController).controller('movieController', movieController);
  ...
  function carousel() {
    return {
      ...
      template: '<div class="jcarousel-wrapper">' + '<div class="jcarousel">' + '<div class="menu-item">' + '<ul>' + '<li ng-repeat="img in images">' + '<div class="menu-image">' + '<div class="poster">' + '<img src="{{img.contentMeta.poster_mage_600x900}}" ' + 'ui-sref="app.menu" style="cursor:pointer; max-width:300px;"/>' + '<img src="{{img.contentMeta.tv_poster_600x900}}" ' + 'ui-sref="app.menu" style="cursor:pointer; max-width:300px;"/>' + '</div>' + '</div>' + '<h2>{{img.name}}</h2>' + '</li>' + '</ul>' + '</div>' + '</div>' + '<a href="javascript:void(0)" class="jcarousel-control-prev">&lsaquo;</a>' + '<a href="javascript:void(0)" class="jcarousel-control-next">&rsaquo;</a>' + '</div>',
  ...
  function tvController($scope, micrositeApi, selector) {
    ...
    var onTvSuccess = function onTvSuccess(data) {
  ...
  function movieController($scope, micrositeApi, selector) {
    void 0;
    var onMovieSuccess = function onMovieSuccess(data) {
      data = _.filter(data, function (item) {
  ...
  function audioController($scope, micrositeApi, selector) {
    ...
    void 0;
    var onAudioSuccess = function onAudioSuccess(data) {
      data = _.filter(data, function (item) {
        return item.contentMeta._highlights === "True";
      });
      void 0;
...
angular.module('poolBear.uploadFileService', []).service('fileUpload', ['$http', function ($http) {
...
(function () {
  ...
  angular.module('poolbear.confirmServices', []).factory('confirmScoreService', ['$rootScope', 'FIREBASE_URL', confirmScoreService]);
  function confirmScoreService($rootScope, FIREBASE_URL) {
    ...
    var confirmScore = function confirmScore(key, note, currentUserId) {
      void 0;
      void 0;
      void 0;
      void 0;
      ...
      var currentUserDetails = void 0; //Current user ref details
      var newUserValueObject = {}; //New user values

      //Player Variable
      var currentPlayerDetails = void 0; //Main player ref details
      ...
      playerRef.once("value", function (snapshot) {
        ...
        if (snapshot.val() === null) {
          ...
          snapshot.ref().update({ "wins": newPlayerValueObject.newWinValuePlayer,
            "lost": newPlayerValueObject.newLostValueUser,
            "point": newPlayerValueObject.newPointsValuePlayer
          });
          void 0;
        }
      ...
      if (confirmMessageRef.confirm === true && confirmMessageRefRecieved.confirm === true) $scope.confirm = true;
    }; // CONFIRM END
...
(function () {
  ...
  angular.module('poolBear.player').factory('firebaseDataService', ['$rootScope', '$firebaseAuth', '$firebaseArray', '$firebaseObject', 'FIREBASE_URL', function ($rootScope, $firebaseAuth, $firebaseArray, $firebaseObject, FIREBASE_URL) {
    ...
    //GET-CURRENT-USER-RESULTS START
    var getCurrentUserResults = function getCurrentUserResults(callback) {
      var ref = new Firebase(FIREBASE_URL);
      var auth = $firebaseAuth(ref);
      auth.$onAuth(function (authUser) {
        if (authUser) {
...
(function () {
  ...
  angular.module('poolbear.setServices', []).factory('setScoreService', ['$rootScope', 'FIREBASE_URL', function confirmScoreService($rootScope, FIREBASE_URL) {
    void 0;
...
(function () {
  ...
  angular.module('poolBear.login').controller('RegistrationController', ['$scope', '$rootScope', 'Authentication', '$state', '$location', '$firebaseArray', 'FIREBASE_URL', '$firebaseObject', '$firebaseAuth', 'fileUpload', 'Upload', '$timeout', '$window', function ($scope, $rootScope, Authentication, $state, $location, $firebaseArray, FIREBASE_URL, $firebaseObject, $firebaseAuth, fileUpload, Upload, $timeout, $window) {
    ...
    auth.$onAuth(function (authUser) {
      if (authUser) {
        ...
        $scope.bearsList = [{ id: 1, name: "poolBear" }, { id: 2, name: "runningBear" }, { id: 3, name: "footballBear" }, { id: 4, name: "squashBear" }];
    ...
    $scope.addbears = function (theme) {
      ...
      theme.forEach(function (item) {
        ...
        var bearAddRef = new Firebase(FIREBASE_URL + 'users/' + $rootScope.currentUser.$id);
        var poolBearUserRef = new Firebase(FIREBASE_URL + 'bears/' + item + '/users');
        var newBearArray = [];
        ...
        bearAddRef.on('value', function (snap) {
          ...
          poolBearUserRef.child($rootScope.currentUser.$id).once("value", function (snapshot) {

            var checkbear = snapshot.val();
            void 0;
...
(function () {
  ...
  angular.module('poolBear').directive('tbLoginForm', function () {
    void 0;
    return {
      restrict: 'E',
...
(function () {
  ...
  angular.module('poolBear').directive('ifLogo', function () {
    return {
      restrict: 'E',
...
(function () {
  ...
  angular.module("poolBear.player").config(function ($stateProvider) {

    $stateProvider.state('app.playersetchallenge', {
  ...
  }).controller('SetChallengeController', ['$scope', '$rootScope', '$firebaseAuth', '$firebaseArray', 'FIREBASE_URL', '$firebaseObject', '$stateParams', '$state', 'confirmScoreService', function ($scope, $rootScope, $firebaseAuth, $firebaseArray, FIREBASE_URL, $firebaseObject, $stateParams, $state, confirmScoreService) {
      ...
      if (authUser) {
        ...
        $scope.addNotification = function (type) {
          ...
          notificationInfoPost.$add({
            ...
            var result = new Firebase(FIREBASE_URL + 'bears/poolbear/notification/' + $rootScope.currentUser.$id + '/' + resultId);
            ...
            notificationInfoRecieve.$add({
              ...
              var resultref = new Firebase(FIREBASE_URL + 'bears/poolbear/notification/' + setScorePlayerUserId + '/' + resultIdref);
...
(function () {
  ...
  angular.module("poolBear.player").config(function ($stateProvider) {
    ...
    $stateProvider.state('app.playersetscore', {
  ...
  }).controller('SetScoreController', ['$scope', '$rootScope', '$firebaseAuth', '$firebaseArray', 'FIREBASE_URL', '$firebaseObject', '$stateParams', '$state', 'confirmScoreService', function ($scope, $rootScope, $firebaseAuth, $firebaseArray, FIREBASE_URL, $firebaseObject, $stateParams, $state, confirmScoreService) {
...
(function () {
  ...
  angular.module('poolBear.player').directive('ifLayout', function () {
...
(function () {
  ...
  angular.module('poolBear.player').directive('tbModal', function () {
    return {
      restrict: 'E',
...
(function () {
  ...
  angular.module('poolBear.player').directive('tbPlayerScore', function () {
    ...
    return {
      ...
      controller: function controller($scope) {
        ...
        var themeName = window.localStorage.getItem("theme");
        void 0;
...
(function () {
  ...
  angular.module('poolBear.player').directive('tbProfilesCardCurrentSmall', function () {
    return {
      restrict: 'E',

      controller: function controller($window, $scope) {
        var themeName = window.localStorage.getItem("theme");
        void 0;
        $scope.theme = themeName;
...
(function () {
  ...
  angular.module('poolBear.player').directive('tbProfilesCardPlayerSmall', function () {
    return {
      restrict: 'E',
      controller: function controller($window, $scope) {
        var themeName = window.localStorage.getItem("theme");
...
(function () {
  ...
  angular.module('poolBear.player').directive('tbProfilesCard', function () {
    return {
      restrict: 'E',
      controller: function controller($window, $scope) {
        var themeName = window.localStorage.getItem("theme");
...
(function () {
  ...
  angular.module('poolBear').directive('angularRating', function () {
    return {
      replace: true,
      require: 'ngModel',
      controller: function controller($scope) {

        //set up and get elo value
        ...
        $scope.fn = function () {
          if ($scope.rating.value === 10) {
            $scope.callbackText = 'This text was changed through the callback function! ' + 'And it was validated to say "YOU\'RE AWESOME" ' + 'if the value was 10!';
            return;
          }
          $scope.callbackText = 'The new rating is ' + $scope.rating.value + '. ' + 'This text was changed through a callback function! ' + 'Try hitting the max value star!';
        };
      },
      scope: {
        ngModel: '=',
      ...
      link: function link(scope, element, attrs, controller) {
        ...
        scope.paintIcons = function (index, reset) {
          ...
          for (var i = 0; i < items.length; i++) {
            ...
            if (index >= i) {
              icon.removeClass(iconEmpty).addClass(reset ? iconFull : iconHover).css('color', reset ? colorSelected : colorHover);
            } else {
              icon.removeClass(iconFull).addClass(iconEmpty).css('color', reset ? colorBase : icon.css('color'));
            }
...
(function () {
  ...
  angular.module('poolBear').directive('tbStatistic', function () {
    return {
      restrict: 'E',
...
'use strict';
angular.module('poolBear.result').config(function ($stateProvider) {
...
angular.module('poolBear.result').config(function ($stateProvider) {
  ...
  $stateProvider.state('app.decline', {
    url: '/player/decline',
    data: { title: 'decline' },
    views: {
      'main': {
...
(function () {
  ...
  angular.module('poolBear.result').directive('tbReceiver', function () {
    return {
      restrict: 'E',
...
(function () {
  ...
  angular.module('poolBear.result').directive('tbSender', function () {
    return {
      restrict: 'E',
