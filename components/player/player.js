...
//               notificationInfoPost.$remove(key);
              ...
              var comfirmId =  confirmedResultObject.resultID; //Get notification userid

              var confirmMessageRef = new Firebase(FIREBASE_URL + 'bears/poolbear/notification/' + postId + "/" + result);


              var confirmMessageRefrecieved = new Firebase(FIREBASE_URL + 'bears/poolbear/notification/' + $rootScope.currentUser.$id + "/" + key);


//             $firebaseArray creates an empty array and placed using the reference create above

              console.log("confirmMessageRef ",confirmMessageRef );
              confirmMessageRef.update({confirmShow:true});

              // confirmedResultObject.update({confirmShow:true});
