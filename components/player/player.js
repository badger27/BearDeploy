        ...
        $scope.confirmScore = function(key,note) {
           ...
           var playerRef = new Firebase(FIREBASE_URL + 'bears/poolbear/users/' + note.playerPostUserId );
            ...
            playerRef.on("value", function(snapshot) {
                ...
                var newLostValueUser =   totalNoGames - note.notificationWinsPost;
                var newLostValuePlayer=   totalNoGames  - note.notificationWinsRecieve;

                console.log(" newLostValueUser: " + newLostValueUser );

             // updates wins for current user
             userRef.update({wins:newWinValueUser, lost:newLostValueUser}), function (err) {
                 if(err){  console.log("err " +err)
                 }
              }
              playerRef.update({wins:newWinValuePlayer,lost:newLostValuePlayer}), function (err) {
                  if(err){  console.log("err " +err)
                  }
              }
