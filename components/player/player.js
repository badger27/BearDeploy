...
//         Add post to reciever 
      ...
      $scope.deleteMeeting = function(key) {
        ...
        $scope.confirmScore = function(key,note) {
            ...
            // To  Do
            // var totalNolostplayer = note.notificationWinsRecieve + note.notificationWinsPost ;
            // var totalNolostUser = note.notificationWinsRecieve + note.notificationWinsPost ;
            // Set lost games player variable
            // Set points gained Variable
            //
           ...
           var playerRef = new Firebase(FIREBASE_URL + 'bears/poolbear/users/' + note.playerPostUserId );
            ...
            playerRef.on("value", function(snapshot) {
                ...
                var newWinValueUser =  currentUserDetails.wins + note.notificationWinsRecieve;
                var newWinValuePlayer=  currentPlayerDetails.wins + note.notificationWinsPost;


            // console.log("newWinValue" + newWinValueUser);
            // console.log( "newWinValuePlayer:" + newWinValuePlayer);

             // updates wins for current user
             userRef.update({wins:newWinValueUser}), function (err) {
                 if(err){  console.log("err " +err)
                 }
              }
              playerRef.update({wins:newWinValuePlayer}), function (err) {
                  if(err){  console.log("err " +err)
                  }
              }
              ...
              // updates wins for other player
              // resultIdref.update({wins:newWinValue}), function (err) {
              //     if(err){  console.log("err " +err)
              //     }
              // }
          ...
          var userPlayerResultRef = new Firebase(FIREBASE_URL + 'bears/poolbear/result/' + $rootScope.currentUser.$id);
...
//                notificationWinsPost: confirmedResultObject.playerPostWinsPost,
                ...
                playerWinsRecieve: note.notificationWinsRecieve,
                playerLostRecieve: note.playerRecieveLost,
                playerFirstname: note.playerPostFirstName,
                playerLastname: note.playerPostlastName

               };


              userPlayerResultRef.child(note.playerPostUserId).set(userPlayerInfo );
